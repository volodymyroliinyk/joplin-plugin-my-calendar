...
// Mock settings to control getIcsImportAlarmsEnabled
...
describe('icsImportService.importIcsIntoNotes', () => {
    ...
    test('if alarms disabled, deletes existing alarms and does not create new ones', async () => {
    ...
    test('if getIcsImportAlarmsEnabled throws, defaults to enabled for backward compatibility', async () => {
        ...
        const joplin = mkJoplin({
            get: jest.fn().mockResolvedValue({items: [], has_more: false}),
            post: jest.fn().mockResolvedValue({id: 'n1'}),
            put: jest.fn().mockResolvedValue({}),
            delete: jest.fn().mockResolvedValue({}),
        });
